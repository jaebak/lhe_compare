...
class ParticleInfo {
  ...
  void clear () {
    for (auto it : m_vint) m_vint[it.first].clear();
    for (auto it : m_vlong) m_vlong[it.first].clear();
    for (auto it : m_vdouble) m_vdouble[it.first].clear();
  }
...
bool isBitTrue(unsigned value, unsigned bit_position) {
  bitset<32> value_bitset(value);
  return value_bitset[bit_position] == 1;
}

bool isParticleFrom(short particle_index, int from_pdgid, TTreeReaderArray<int> & GenPart_pdgId, TTreeReaderArray<short> & GenPart_genPartIdxMother) {
  int mother_pdgid = -1;
  short mother_index = GenPart_genPartIdxMother[particle_index];
  if (mother_index != -1) mother_pdgid = GenPart_pdgId[mother_index];
  if (mother_pdgid == from_pdgid) return true;
  if (mother_pdgid != -1) return isParticleFrom(mother_index, from_pdgid, GenPart_pdgId, GenPart_genPartIdxMother);
  else return false;
}
  ...
  while (reader.readEvent()) {
    ...
    for (unsigned iParticle = 0; iParticle < nParticles; ++iParticle) {
      ...
      for (unsigned jParticle = iParticle+1; jParticle < nParticles; ++jParticle) {
        ...
        particle_info.m_vint["status_code"].push_back(-1);
        ...
        particle_info.m_vdouble["lifetime"].push_back(-1);
        particle_info.m_vdouble["cos_spin_decay"].push_back(-1);
        particle_info.m_vint["mother_idx_first"].push_back(-1);
        particle_info.m_vint["mother_idx_second"].push_back(-1);
        particle_info.m_vint["colors_first"].push_back(-1);
        particle_info.m_vint["colors_second"].push_back(-1);
        // Set hist definition
...
void fill_tree_from_nano(TTree * nano_tree, TTree * tree, ParticleInfo & particle_info, map<string, tuple<string, long, double, double> > & hist_definitions, bool verbose = false) {
  ...
  // https://gist.github.com/jpata/7ed5447734990954038bbb6fb6b7f9d3
  TTreeReader reader(nano_tree);
  TTreeReaderArray<float> GenPart_eta(reader, "GenPart_eta");
  TTreeReaderArray<short> GenPart_genPartIdxMother(reader, "GenPart_genPartIdxMother");
  ...
  TTreeReaderArray<unsigned short> GenPart_statusFlags(reader, "GenPart_statusFlags");
  TTreeReaderValue<float> Generator_weight(reader, "Generator_weight");
  ...
  while (reader.Next()) {
    ...
    // Reset storage
    particle_info.clear();
    unsigned nParticles = GenPart_pdgId.GetSize();
    particle_info.resize(nParticles);

    // Set event variables
    particle_info.m_double["weight"] = *Generator_weight;

    // Loop over all particles to set particle variables
    for (unsigned iParticle = 0; iParticle < GenPart_pdgId.GetSize(); ++iParticle) {
      // Only allow particles from the hard process
      if (isBitTrue(GenPart_statusFlags[iParticle], 7) == 0) continue;
      // Ignore particles that are comming from the Higgs boson
      if (isParticleFrom(iParticle, 25, GenPart_pdgId, GenPart_genPartIdxMother) == 1) continue;
      //if (GenPart_pdgId[iParticle] == -2) cout<<"iEvent: "<<iEvent<<" iParticle: "<<iParticle<<" pdgId: "<<GenPart_pdgId[iParticle]<<" IdxMother: "<<GenPart_genPartIdxMother[iParticle]<<" status: "<<GenPart_status[iParticle]<<" statusFlags: "<<GenPart_statusFlags[iParticle]<<" pt: "<<GenPart_pt[iParticle]<<endl;


      //cout<<"gen eta["<<iParticle<<"]: "<<GenPart_eta[iParticle]<<endl;
      particle_info.m_vlong["pid"][iParticle] = GenPart_pdgId[iParticle];
      particle_info.m_vint["status_code"][iParticle] = GenPart_status[iParticle];
      particle_info.m_vdouble["px"][iParticle] = GenPart_pt[iParticle]*cos(GenPart_phi[iParticle]);
      particle_info.m_vdouble["py"][iParticle] = GenPart_pt[iParticle]*cos(GenPart_phi[iParticle]);
      particle_info.m_vdouble["pz"][iParticle] = GenPart_pt[iParticle]*sinh(GenPart_eta[iParticle]);
      particle_info.m_vdouble["mass"][iParticle] = GenPart_mass[iParticle];
      particle_info.m_vdouble["energy"][iParticle] = sqrt(pow(GenPart_pt[iParticle]*cosh(GenPart_eta[iParticle]),2)+pow(GenPart_mass[iParticle],2));
      particle_info.m_vdouble["lifetime"][iParticle] = -1;
      particle_info.m_vdouble["cos_spin_decay"][iParticle] = -1;

      particle_info.m_vint["mother_idx_first"][iParticle] = GenPart_genPartIdxMother[iParticle];
      particle_info.m_vint["mother_idx_second"][iParticle] = GenPart_genPartIdxMother[iParticle];

      particle_info.m_vint["colors_first"][iParticle] = -1;
      particle_info.m_vint["colors_second"][iParticle] = -1;
    ...
    for (unsigned iParticle = 0; iParticle < nParticles; ++iParticle) {
      ...
      } // Loop over j particle
    } // Loop over i particle
...
int main(int argc, char *argv[]){
  ...
  string path_a = lhe_a;
  string path_b = nano_b;

  LHEF::Reader * reader_a = new LHEF::Reader(path_a);
  TChain * nano_tree_b = new TChain("Events");
  nano_tree_b->Add(path_b.c_str());
  ...
  if (path_b != "") {
    ...
    fill_tree_from_nano(nano_tree_b, tree_b, particle_info_b, hist_definitions);
    tree_b->Write();
  }
  ...
  if (path_b != "") fill_histograms(tree_b, hist_definitions, histograms_b, nbins, /*tag*/ tag_b);
  ...
  for (string const & key : keys) {
    ...
    if (path_b != "") {
      ...
      if (fabs(x_max - x_min)>0.000001) { // Prevent residuals for one bin histograms
        ...
        char c_pvalue[64]; snprintf(c_pvalue, sizeof c_pvalue, "#chi^{2} test (%s vs %s) p-value: %.3g", lhe_a_tag.c_str(), tag_b.c_str(), pvalue);
